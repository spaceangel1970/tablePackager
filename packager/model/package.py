...
class Manifest:
    ...
    def new(self) -> None:
        ...
        self.content['visual pinball']['info']['ultraDMD'] = ''
        self.content['visual pinball']['info']['flexDMD'] = ''
...
class Package:
    ...
    def new(self, package_dir):
        ...
        self.__manifest = Manifest(self._name, self.baseModel.package_version)
        self.__manifest.new()  
    ...
    def open(self, base_dir):
        ...
        bad_workspace_path = os.path.join(self.directory, self._name, 'media', 'PuP Pack')
    ...
    def check_package(self) -> None:
        ...
        file_in_error = self.check_files(self.manifest.content, self.directory + '/' + self._name, '')
    ...
    def upgrade_package(self):
        ...
        if major_package_version == 1 and (minor_package_version == 0 or minor_package_version == 1):
            ...
            if not os.path.exists(self.directory + '/' + self._name + '/media/TopperVideos'):
                os.makedirs(self.directory + '/' + self._name + '/media/TopperVideos')
            self.manifest.content['media']['Loading'] = []
            if not os.path.exists(self.directory + '/' + self._name + '/media/Loading'):
                os.makedirs(self.directory + '/' + self._name + '/media/Loading')
    ...
    def merge(self, installed: bool = False) -> None:
        installed_manifest = Manifest(self._name, self.baseModel.package_version)
        installed_manifest.open(self.__baseModel.installed_path, installed)

        self.merge_files(installed_manifest.content, self.directory + '/' + self._name, '')
    ...
    def rename_files(self, new_name, path, content):
        for key, value in content.items():
            if key == 'info':
                ...
                if value['name'].startswith(self._name, 0, len(self._name)):
                    self.logger.info("+ rename file '%s'" % (path + '/' + value['name']))
                    os.rename(self.directory + '/' + self._name + '/' + path + '/' + value['name'],
                              self.directory + '/' + self._name + '/' + path + '/' + value['name'].replace(self._name,
                                                                                                         new_name))
